...
mod tests {
    ...
    #[test]
    fn test_sampling_overrides_with_values() {
        let overrides = SamplingOverrides {
            temperature: Some(0.5),
            top_p: Some(0.9),
        };
        assert!(overrides.temperature.is_some());
        assert_eq!(overrides.temperature.unwrap(), 0.5);
    }

    #[test]
    fn test_sampling_overrides_serialization() {
        let overrides = SamplingOverrides {
    ...
    #[test]
    fn test_sampling_overrides_deserialization() {
        let json = r#"{"temperature": 0.4, "topP": 0.8}"#;
...
mod model_status_tests {
    ...
    fn test_model_status_healthy() {
        ...
        assert!(status.model_name.is_some());
    }
    ...
    fn test_model_status_serialization() {
        let status = ModelStatus {
            ...
            healthy: true,
            model_name: Some("qwen2.5:14b".to_string()),
            error: None,
        };
    ...
    fn test_model_status_deserialization() {
        ...
        assert_eq!(status.key, "lm-studio-model");
        assert!(status.healthy);
        assert_eq!(status.model_name, Some("model-name".to_string()));
    }

    #[test]
    fn test_model_status_with_error_deserialization() {
        let json = r#"{
            "key": "ollama-model",
        ...
        let status: ModelStatus = serde_json::from_str(json).unwrap();
        assert!(!status.healthy);
        assert_eq!(status.error, Some("timeout after 30s".to_string()));
    }
